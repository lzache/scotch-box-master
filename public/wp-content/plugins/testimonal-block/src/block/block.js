/**
 * BLOCK: testimonal-block
 *
 * Registering a basic block with Gutenberg.
 * Simple block, renders and saves the same content without any interactivity.
 */

//  Import CSS.
import './editor.scss';
import './style.scss';

const { __ } = wp.i18n; // Import __() from wp.i18n
const { registerBlockType } = wp.blocks; // Import registerBlockType() from wp.blocks
const { RichText, PlainText, MediaUpload, MediaUploadCheck } = wp.blockEditor;
const { SelectControl } = wp.components;


/**
 * Register: aa Gutenberg Block.
 *
 * Registers a new block provided a unique name and an object defining its
 * behavior. Once registered, the block is made editor as an option to any
 * editor interface where blocks are implemented.
 *
 * @link https://wordpress.org/gutenberg/handbook/block-api/
 * @param  {string}   name     Block name.
 * @param  {Object}   settings Block settings.
 * @return {?WPBlock}          The block, if it has been successfully
 *                             registered; otherwise `undefined`.
 */
registerBlockType( 'lz-plugin/testimonal-block', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __( 'Testimonial' ), // Block title.
	icon: 'thumbs-up', // Block icon from Dashicons → https://developer.wordpress.org/resource/dashicons/.
	category: 'common', // Block category — Group blocks together based on common traits E.g. common, formatting, layout widgets, embed.
	keywords: [
		__( 'testimonal' ),
	],

	attributes: {
		quote: {
			type: 'string',
			source: 'html',
			selector: '.quote',
		},

		author: {
			type: 'string',
			source: 'text',
			selector: '.author',
		},

		stars: {
			type: 'number',
			default: 5,
		},

		imgUrl: {
			type: 'string',
			default: 'https://placehold.it/75',
		},

		location: {
			type: 'string',
			source: 'html',
			selector: '.location',
		},
	},

	/**
	 * The edit function describes the structure of your block in the context of the editor.
	 * This represents what the editor will render when the block is used.
	 *
	 * The "edit" property must be a valid function.
	 *
	 * @link https://wordpress.org/gutenberg/handbook/block-api/block-edit-save/
	 *
	 * @param {Object} props Props.
	 * @returns {Mixed} JSX Component.
	 */
	edit: ( props ) => {
		let {attributes: { quote, author, stars, imgUrl, location }, setAttributes, className } = props;

		// creating method to save the quote
		function changeQuote(value){
			setAttributes({quote: value});
		}

		function changeAuthor(value){
			setAttributes({author: value});
		}

		function changeStars(value){
			setAttributes( {stars: parseInt(value) } );
		}

		function selectImage(value){
			// console.log(value);
			// ^^ will show you properties of the file that was added
			setAttributes( {imgUrl: value.sizes.thumbnail.url } );   // thumbnail, full, medium
		}

		function changeLocation(value){
			setAttributes({location: value});
		}

		// Creates a <p class='wp-block-cgb-block-testimonal-block'></p>.
		return (
			<div className={ props.className }>
				<SelectControl className="stars"
							label=""
							value={stars}
							onChange={changeStars}
							options={[
								{label: '*', value:1 },
								{label: '**', value:2 },
								{label: '***', value:3 },
								{label: '****', value:4 },
								{label: '*****', value:5 },
							]}
				/>

				<RichText className="quote"
						  tagName="div"
						  value= {quote}
						  onChange={changeQuote}
						  placeholder="This is a place holder for the text "/>

						  <div className="quote-profile">
							  <div className="photo">
							  <MediaUploadCheck>
								  <MediaUpload
								  onSelect={selectImage}
								  // render is what do you want the user to click on to open the uploader
								  render={({open}) =>
								  	<img src={imgUrl} onClick={open} />
								  }
								  	/>
							  </MediaUploadCheck>
							  </div>


							  <div className="text">
								  <PlainText className="author"
											 value= {author}
											 onChange= {changeAuthor}
											 placeholder="Eric Forman" />

							  </div>
							  <div className="location">
								  <PlainText className="location"
											 value= {location}
											 onChange= {changeLocation}
											 placeholder="Here" />

							  </div>
						  </div>
			</div>
		);
	},

	/**
	 * The save function defines the way in which the different attributes should be combined
	 * into the final markup, which is then serialized by Gutenberg into post_content.
	 *
	 * The "save" property must be specified and must be a valid function.
	 *
	 * @link https://wordpress.org/gutenberg/handbook/block-api/block-edit-save/
	 *
	 * @param {Object} props Props.
	 * @returns {Mixed} JSX Frontend HTML.
	 */
	save: ( props ) => {
		let starIcons = Array(5).fill('&#9733;', 0, props.attributes.stars).join('');

		return (
			<div className={ props.className }>
				<div className="stars">{starIcons}</div>
				<RichText.Content tagName="div"
								  className="quote"
								  value={props.attributes.quote}/>
				<div className="quote-profile">
					<div className="photo">
						<img src={props.attributes.imgUrl} />
					</div>
					<div className="text">
						<p className="author"> {props.attributes.author} </p>

						<p className="location"> {props.attributes.location} </p>

					</div>
				</div>


			</div>
		);
	},
} );
